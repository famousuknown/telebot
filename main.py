...
ELEVENLABS_API_KEY = os.getenv("ELEVEN_API_KEY")  # исправлено на правильное имя
...
# Функция генерации речи клонированным голосом

async def generate_cloned_tts(voice_id: str, text: str):
    ...
    if response.status_code == 200:
        ...
        print(f"❌ Ошибка TTS: {response.text}")
            ...
            if mode == "mode_voice":
                ...
                if not voice_id:
                    ...
                    if duration_sec < 30:
                        ...
                        await update.message.reply_text("⚠️ Для клонирования голоса требуется минимум 30 секунд аудио.")

                        return

                    await update.message.reply_text("⏳ Клонирую голос...")
                    ...
                    if not voice_id:
                        ...
                        await update.message.reply_text("❌ Ошибка клонирования голоса.")

                        return

                    context.user_data["cloned_voice_id"] = voice_id

                    await update.message.reply_text("✅ Голос успешно клонирован!")



                # Генерируем перевод клонированным голосом

                tts_audio = await generate_cloned_tts(voice_id, translated)

                if tts_audio:

                    await update.message.reply_voice(voice=tts_audio, reply_markup=back_button_markup)

                else:

                    await update.message.reply_text("❌ Ошибка генерации речи.")
